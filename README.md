...
| `guard-cloudflare-assets.sh` | Stops blanket `run_worker_first = true` on Workers Static Assets. Blanket true bills every `/js` `/css` `/fonts` hit as a Workers request.<br>**Agent action:** Sets `run_worker_first = true` under `[assets]` in `wrangler.toml`.<br>**Guard action:** Rejects blanket true; allows `false` or a selective path array so free assets skip the Worker. |
| `guard-edge-shield.sh` | Stops reintroducing a front-door proxy Worker (e.g. `radioindex-edge-shield`) on www/apex in front of `app`. A second Worker on the hot path double-bills every request.<br>**Agent action:** Adds `workers/*-edge-shield/`, `deploy:edge-shield`, or AGENTS text that routes www through a proxy Worker.<br>**Guard action:** Rejects the name/path/workflow and requires www/apex → `app` only. |
